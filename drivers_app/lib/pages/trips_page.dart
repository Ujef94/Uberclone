...
class _TripsPageState extends State<TripsPage> {
  ...
  {
    DatabaseReference tripsRequestsRef = FirebaseDatabase.instance.ref().child("tripRequests");

    await tripsRequestsRef.once().then((snap)
    {
        if(snap.snapshot.value != null)
          {
            ...
            List<String> tripCompletedByCurrentDriver = [];
              ...
              if(value["status"] != null)
                {
                  if(value["status"] == "ended")
                  {
                      if(value["driverID"] == FirebaseAuth.instance.currentUser!.uid)
                        {
                          tripCompletedByCurrentDriver.add(key);
                        }
                  }
                }
            });
            ...
            setState(() {
              currentDriverTotalTripsCompleted = tripCompletedByCurrentDriver.length.toString();
            });
          }
    });
  }
